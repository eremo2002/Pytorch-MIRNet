...
def train():
    ...
    train_transform = transforms.Compose([Random_Brightness(p=0.5, sigma1=0.3),                                                                      
                                        ...
                                        Rotation(p=0.5,
                                                angle=(-30, 30)),                                        
                                        Random_Crop(patch_size=(128, 128)), # for patch-wise training                                        
                                        Normalize(),
                                        ToTensor()])
    ...
    model = MIRNet(in_channels=1, out_channels=1, num_features=32)
    model.to(device)
    summary(model,(1, 128, 128), batch_size=BATCH_SIZE)
    
    criterion = Charbonnier_Loss().to(device)
    ...
    for epoch in range(1, EPOCHS+1):
        ...
        if best_val_loss > val_loss:            
            ...
            torch.save({'epoch': epoch,
                        ...
                        'loss': criterion},
                        f'weight/MIRNet_{epoch:04d}_valloss_{val_loss:.4f}.pth')
